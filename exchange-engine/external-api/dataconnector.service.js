...
export const runConnectorProcess = async (
  ...
  try {
    const dataSourceCollection = await findOneCollectionService(projectId, collectionName);
    ...
    if (refTypeFields && refTypeFields.length > 0) {
      ...
      if (Object.keys(refKeyMapping).length > 0) {
        for (let key in refKeyMapping) {
          refCollection[key] = await findOneCollectionService(
            projectId,
            refKeyMappingForCollection[key],
          );
        }
      }
    }
