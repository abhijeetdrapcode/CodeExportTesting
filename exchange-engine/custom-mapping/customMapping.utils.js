import { mergeObjects } from 'drapcode-utility';
...
const convertRefStrToArr = (item, mappedData, mapping, refArrayFormat) => {
  ...
  mapping.forEach((obj) => {
    if (obj.value.includes('RF::')) {
      if (!obj.value.includes('.')) {
        ...
        referenceFieldsKeys.push({ key: obj.key, value });
  ...
  Object.keys(mappedData).forEach((key) => {
    ...
    let refObj = referenceFieldsKeys.find((obj) => obj.key === key);
    if (referenceFieldsKeys.length && refObj) {
      finalData[key] = item[refObj.value];
