...
export const processFieldsInclude = (finder, data, user, tenant) => {
  ...
  if (
    ...
    if (finder.fieldsInclude.some((incl) => incl.startsWith('DF::')))
      data = addDerivedFieldInclude(finder, data, user, tenant);
...
const addDerivedFieldInclude = (finder, data, user, tenant) => {
  const isArray = Array.isArray(data);
  let result = data;
  if (result) {
    result = isArray ? result : [result];
    ...
    derivedFieldsInclude.forEach((exField) => {
      ...
      if (derivedField) {
        result = result.map((item) => {
          const value = prepareFunction({ functionDef: derivedField, field: item, user, tenant });
...
export const checkDerivedFieldMapping = async (
  ...
  const updatedItems = items.map((item) => {
    ...
    mapping.forEach(({ derivedField, fieldName }) => {
      const result = prepareFunction({ functionDef: derivedField, field: newItem, user });
